...
public class Preconditions {
    ...
    private static String badElementIndex(int index, int size, String desc) {
        if (index < 0) {
            return format("%s (%s) must not be negative", new Object[]{desc, Integer.valueOf
                    (index)});
        } else if (size < 0) {
            throw new IllegalArgumentException((new StringBuilder(26)).append("negative size: ")
                    .append(size).toString());
        } else {
            return format("%s (%s) must be less than size (%s)", new Object[]{desc, Integer
                    .valueOf(index), Integer.valueOf(size)});
        }
    }
    ...
    private static String badPositionIndex(int index, int size, String desc) {
        if (index < 0) {
            ...
            return format("%s (%s) must not be greater than size (%s)", new Object[]{desc,
